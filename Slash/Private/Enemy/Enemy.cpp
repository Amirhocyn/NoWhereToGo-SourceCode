...
{
	EnemyContoller = Cast<AAIController>(GetController());
...
bool AEnemy::InTargetRange(AActor* Target, double Radius)
{
	...
	//if(GEngine)
		//GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, FString::Printf(TEXT("%f"), (Target->GetActorLocation() - GetActorLocation()).Size()));
	return DistanceToTarget <= Radius;
}

void AEnemy::MoveToTarget(AActor* Target)
{
	if (EnemyContoller == nullptr || Target == nullptr) return;
	FAIMoveRequest MoveRequest;
	MoveRequest.SetGoalActor(Target);
	MoveRequest.SetAcceptanceRadius(AcceptanceRadius);
	EnemyContoller->MoveTo(MoveRequest);
}
